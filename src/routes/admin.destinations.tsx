...
function AdminDestinationsPage() {
  ...
  const saveMut = useMutation({
    mutationFn: (data: typeof emptyForm) =>
      editId !== null
        ? api.patch(`/api/destinations/${editId}`, data)
        : api.post("/api/destinations", data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["destinations"] }); setModal(null); },
  });

  const deleteMut = useMutation({
    mutationFn: (id: number) => api.delete(`/api/destinations/${id}`),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["destinations"] }); setDeleteId(null); },
  });
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className={labelCls}>Slug</label><input className={inputCls} placeholder="e.g. maldives" value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} /></div>
              ...
              <div>
                ...
                <select className={inputCls} value={form.budgetBand} onChange={(e) => setForm({ ...form, budgetBand: e.target.value })}>
              ...
              <div className="flex flex-wrap gap-2">
                {tripTypeOptions.map((t) => (
                  <button key={t} type="button" onClick={() => toggleTripType(t)}
                    className={cn("rounded-full px-3 py-1 text-xs font-medium transition-colors", form.tripType.includes(t) ? "bg-[#042045] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200")}>
                    {t}
                  </button>
                ))}
              </div>
