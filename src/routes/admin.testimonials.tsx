...
import { reviews as initialReviews, aggregate } from "@/data/reviews";
...
const emptyForm = { author: "", trip: "", rating: 5 as 4 | 5, date: new Date().toISOString().slice(0, 10), body: "" };
...
function AdminTestimonialsPage() {
  const [items, setItems] = useState<ReviewItem[]>(initialReviews.map(toItem));
  ...
  const handleSave = () => {
    if (modal === "add") {
      const id = `r${Date.now()}`;
      setItems((prev) => [{ ...form, id, _id: id }, ...prev]);
    } else if (editId) {
    ...
    <div className="p-6 lg:p-8">
        ...
        {items.map((r) => (
          <div key={r._id} className="flex flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-3">
              <span className="text-xs text-gray-400">{new Date(r.date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
              <div className="flex items-center gap-1.5">
                <button onClick={() => openEdit(r)} className="rounded-lg border border-gray-200 p-1.5 text-gray-400 hover:bg-gray-50 hover:text-gray-600"><Pencil className="h-3.5 w-3.5" /></button>
                <button onClick={() => setDeleteId(r._id)} className="rounded-lg border border-gray-200 p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-500"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      <Dialog open={modal !== null} onOpenChange={(o) => !o && setModal(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{modal === "add" ? "Add testimonial" : "Edit testimonial"}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div><label className={labelCls}>Author</label><input className={inputCls} placeholder="e.g. Sarah & James M." value={form.author} onChange={(e) => setForm({ ...form, author: e.target.value })} /></div>
            <div><label className={labelCls}>Trip</label><input className={inputCls} placeholder="e.g. Maldives honeymoon, 14 nights" value={form.trip} onChange={(e) => setForm({ ...form, trip: e.target.value })} /></div>
            <div>
              <label className={labelCls}>Rating</label>
              <div className="flex items-center gap-2">
                {[4, 5].map((n) => (
                  <button key={n} type="button" onClick={() => setForm({ ...form, rating: n as 4 | 5 })}
                    className={`flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-sm font-medium transition-colors ${form.rating === n ? "border-amber-400 bg-amber-50 text-amber-700" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                    <Star className={`h-4 w-4 ${form.rating === n ? "fill-amber-400 text-amber-400" : "text-gray-300"}`} />{n} stars
                  </button>
                ))}
              </div>
            </div>
            ...
            <div><label className={labelCls}>Review text</label><textarea className={inputCls} rows={4} value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={deleteId !== null} onOpenChange={(o) => !o && setDeleteId(null)}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-gray-500">This review will be removed from the site.</p>
          <DialogFooter>
            <DialogClose asChild><button className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">Cancel</button></DialogClose>
