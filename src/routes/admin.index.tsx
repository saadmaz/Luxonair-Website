...
const stats = [
  { label: "Total Enquiries", value: "24", change: "+12% this month", positive: true, icon: FileText, color: "text-blue-600", bg: "bg-blue-50" },
  { label: "New This Week", value: "7", change: "3 awaiting reply", positive: true, icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50" },
  { label: "Subscribers", value: "152", change: "+8 this week", positive: true, icon: Users, color: "text-violet-600", bg: "bg-violet-50" },
  { label: "Unread Messages", value: "3", change: "Oldest: 2 days ago", positive: false, icon: MessageSquare, color: "text-amber-600", bg: "bg-amber-50" },
];
        ...
        {stats.map(({ label, value, change, positive, icon: Icon, color, bg }) => (
          <div key={label} className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className={cn("rounded-lg p-2", bg)}><Icon className={cn("h-4 w-4", color)} strokeWidth={1.75} /></div>
            </div>
            <p className="mt-3 text-3xl font-bold text-gray-900">{value}</p>
            <p className={cn("mt-1 text-xs", positive ? "text-emerald-600" : "text-amber-600")}>{change}</p>
          </div>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {recentEnquiries.map((e) => (
                <tr key={e.id} className="transition-colors hover:bg-gray-50/60">
                  ...
                  <td className="px-4 py-4 text-gray-500">{e.party} {e.party === 1 ? "person" : "people"}</td>
          ...
          {recentEnquiries.map((e) => (
            <div key={e.id} className="px-5 py-4">
              ...
              <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500">
                ...
                <span>{e.travelDate}</span><span>{e.budget}</span>
